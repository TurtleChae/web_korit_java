...
public class Practice02 {
    public static void main(String[] args) {
        ...
        List<Product> products = Arrays.asList(
                new Product(1, "삼성 객럭시 북", true, 5, 2000000),
                new Product(2, "애플 m1", false, 1, 15900000),
                new Product(3, "삼성 키보드", true,10, 40000),
                new Product(4, "애플 펜슬", true, 3, 1900000),
        ...
        // 세일 중인 상품을 리스트로 추출
        // 단, 세일 일 경우, 정가에서 *0,9해서 추출
        // 예) 삼성 갤럭시 북 2000000 -> 1800000
        List<Product> result2 = products.stream()
                .filter(p -> !p.getProductname().contains("애플"))
                // 메서드 참조 (레퍼런스)
                // 람다식 축약 버전. 클래스명 :: 메서드명 or 객체명 :: 메서드명
                // a -> System.out.println(a)
                // System.out::println
                // 받아서 그대로 메서드만 호출하는 경우에 사용
        ...
        // 4. 세일 중인 상품들의 총 가치(재고 * 가격) 계산해서 출력
