...
public class If {
    public static void main(String[] args) {
        ...
        } else { // 위의 조건들을 모두 false일때
            System.out.println("미성년자입니다.");
        }

        // if ~ else if ~ else문
        // 위에서부터 아래로 검사함 (동시에 여러조건을 검사하는 것이 x)
        // true가 연산되면, 해당 블럭코드를 실행하고 탈출
        // 오름차순, 내림차순으로 작성해야한다!
        age = 12;
        if (age <= 7) {
            System.out.println("유치원갑시다.");
        ...
        // 내가 정한 규칙(비지니스로직)에서 조건문을 작성할 때
        boolean isLogin = false; // 로그인 x
        boolean isBanned = true; // 밴계정

        if (!isLogin) { // 로그아웃이라면
            System.out.println("로그인이 필요합니다.");
        } else if (isBanned) {
            // 위 조건이 false일때 실행 -> 로그인일때만 실행
        ...
        // 실습2) num2, num3 중 큰 값으 출력
        int num2 = 10;
        int num3 = 5; //20으로 바뀌면 20으로 출력
        if (num2 > num3) {
            System.out.println(num2);
        } else if (num3 > num2) {
            System.out.println(num3);
        } else {
            System.out.println("num2와 num3은 같다.");
        }
        ...
        // 25 이상 30 미만 -> 과체중
        // 30 이상 -> 비만
        // 철수(174, 70)의 비만도결과 출력해주세요.
        double height1 = 174 / 100.0; // int와 double이 연산되면 결과는 double
        // cm를 meter로 변환
        double weight = 70;
        // 연산자 우선순위 : () > 대입
        // 우선순위 동일하면, 왼쪽부터 연산
